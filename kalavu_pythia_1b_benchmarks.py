...
"""
KALAVU: Pythia-1B Downstream Benchmarks
========================================
...
def main():
    print("=" * 70)
    print("KALAVU: Pythia-1B Downstream Benchmarks")
    print("=" * 70)
    ...
    for domain in DOMAINS:
        ...
        if not ckpt.exists():
            raise FileNotFoundError(
                ...
                f"Run kalavu_pythia_1b_experiment.py first."
            )
    ...
    if moe_avg is not None and base_avg is not None:
        print(f"\nGit commit message:")
        print(f"  [kalavu] 1B benchmarks: MoE avg={moe_avg:.1f}% vs base avg={base_avg:.1f}%")
