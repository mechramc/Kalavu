...
"""
KALAVAI Results Writeup — Full PDF
...
OUT     = Path("KALAVAI_Results_Writeup.pdf")
...
story += [
    Spacer(1, 0.8 * cm),
    Paragraph(
        "KALAVAI is a decentralized LLM training protocol where independent GPU "
...
story.append(Paragraph(
    "The primary result validates the KALAVAI mechanism on a real pre-trained model. "
...
story += [
    ...
    Paragraph(
        ...
        f"This result directly supports the KALAVAI cooperative training protocol: "
...
story += [Paragraph("7. Maturity Sweep: When Does Fusion Help?", H1), hr(),
          Paragraph(
              ...
              "'KALAVAI window' where fusion is beneficial.", Body)]
...
story += [
    ...
    Paragraph(
        ...
        "pre-training corpus), bounding the regime where KALAVAI applies.", Body),
    fig("fig_maturity_curve_combined.png", width_cm=15,
        caption="Figure 12. Combined maturity curves: Pythia-410M (blue circles) and Pythia-1B (red squares). "
                ...
                "diminishing returns at full training. Gold shading = KALAVAI target regime (0–20%)."),
...
story += [Paragraph("11. Results Integrity Audit", H1), hr(),
          Paragraph(
              ...
              "(kalavai_results_audit.py) covering: file presence, mathematical verification "
...
story.append(table([
    ...
    ["KALAVAI_Results_Writeup.pdf",                   "This document"],
...
story += [
    ...
    Paragraph(
        ...
        "Evaluation: held-out packed chunks only. Git repository: github.com/mechramc/Kalavai.",
...
def on_page(canvas, doc):
    ...
    canvas.drawString(2*cm, 1.2*cm, "KALAVAI — Murai Labs — March 2026")
