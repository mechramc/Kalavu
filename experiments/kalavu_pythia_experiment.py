...
"""
KALAVU: Pythia-410M Three-Domain Specialist Fusion Experiment
=============================================================
Loads Pythia-410M at step10000 (~7% through training), trains three domain
...
Paper narrative:
  ...
  - Pythia (early training):      +X.X%   <- transfers to real models
  - Qwen (fully trained):         -1.0%   <- diminishes with base knowledge

Hypothesis: Pythia at step10000 knows basic English but has shallow domain
...
SEQ_LEN = 512
WARMUP_FRACTION = 0.1
N_SAMPLES_PER_DOMAIN = 3000   # enough for >2000 train chunks each
ROUTER_STEPS = 500
ROUTER_LR = 1e-3
ROUTER_BATCH = 4
EVAL_BATCHES = 50
...
# ============================================================================
# PackedChunkDataset — copied from kalavu_qwen_divergent_domains.py
# ============================================================================
...
def save_training_curves(loss_histories: dict, seed: int):
    """fig_training_curves_seed42.png: 3-line loss plot."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        ax.set_title(f"Pythia-410M Specialist Training Curves (seed={seed})")
        ax.legend()
        ax.grid(True, alpha=0.3)
        fig.tight_layout()

        FIGURES_DIR.mkdir(parents=True, exist_ok=True)
        path = FIGURES_DIR / f"fig_training_curves_seed{seed}.png"
...
def save_divergence_heatmap(loss_matrix: dict, seed: int):
    """fig_divergence_heatmap.png: 4×3 heatmap (rows=models, cols=domains)."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        ax.set_title(f"Divergence Heatmap — Held-Out Losses (seed={seed})")
        ...
        FIGURES_DIR.mkdir(parents=True, exist_ok=True)
        path = FIGURES_DIR / "fig_divergence_heatmap.png"
        fig.savefig(path, dpi=150)
        plt.close(fig)
        print(f"  Saved: {path}")
    except Exception as e:
...
def save_fusion_comparison(fusion_results: dict, base_losses: dict, seed: int):
    """fig_fusion_comparison.png: grouped bar chart."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        ax.set_title(f"Fusion Comparison — Held-Out Eval (seed={seed})")
        ax.legend(loc="upper right", fontsize=8)
        ax.grid(True, axis="y", alpha=0.3)
        fig.tight_layout()

        FIGURES_DIR.mkdir(parents=True, exist_ok=True)
        path = FIGURES_DIR / "fig_fusion_comparison.png"
        fig.savefig(path, dpi=150)
        plt.close(fig)
        print(f"  Saved: {path}")
    except Exception as e:
...
def save_router_distribution(router_dist: dict, seed: int):
    """fig_router_distribution.png: 3×3 bar chart of router weights per domain."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        fig.suptitle(f"Router Gate Distribution by Input Domain (seed={seed})")
        fig.tight_layout()

        FIGURES_DIR.mkdir(parents=True, exist_ok=True)
        path = FIGURES_DIR / "fig_router_distribution.png"
        fig.savefig(path, dpi=150)
        plt.close(fig)
        print(f"  Saved: {path}")
    except Exception as e:
...
def step0_env_check():
    ...
    assert 390e6 <= total_params <= 430e6, f"Unexpected param count: {total_params/1e6:.1f}M"
    ...
    if quick_loss > 8.0:
        ...
        print("Loss in acceptable range ✓")
...
def step1_base_eval(tokenizer, device: str) -> dict:
    ...
    for d, l in base_losses.items():
        ok = "✓" if 2.0 <= l <= 8.0 else "⚠ UNEXPECTED"
        print(f"  {d}: {l:.4f} {ok}")
...
def step2_train_specialists(tokenizer, all_domain_chunks: dict, device: str) -> dict:
    print("\n" + "=" * 70)
    print("STEP 2: Train Specialists (3 seeds × 3 domains = 9 runs)")
    print("=" * 70)
    ...
    for seed in SEEDS:
        ...
        for domain in DOMAINS:
            ...
            # Save only state_dict of unfrozen params to save space
            ...
            # Actually save full state_dict for easy reloading; unfrozen-only
            # would require knowing frozen keys at load time
...
def step3_divergence_check(tokenizer, all_domain_chunks: dict, device: str) -> dict:
    print("\n" + "=" * 70)
    print("STEP 3: Divergence Check (3×3 matrix per seed)")
    print("=" * 70)
    ...
    if any_failed:
        print("\nWARNING: One or more seeds failed divergence check.")
        print("Proceeding to fusion step but results may be unreliable.")
...
def step4_fusion(tokenizer, all_domain_chunks: dict, device: str,
    ...
    for seed in SEEDS:
        ...
        eval_domains = list(held_out_sets.keys())
        ...
        out_path = RESULTS_DIR / f"step4_fusion_results_seed{seed}.json"
        with open(out_path, "w") as f:
            json.dump(seed_result, f, indent=2)
        print(f"\nSaved: {out_path}")
...
def step5_final_summary(step1_result: dict, step3_results: dict,
    ...
    print(f"\nINTERPRETATION:")
    if mean_imp > 20:
        print(f"  A: Strong result ({mean_imp:.1f}%) — fusion generalizes well to early-training real models.")
    elif mean_imp > 5:
        print(f"  B: Modest result ({mean_imp:.1f}%) — fusion works incrementally on early-training real models.")
    elif mean_imp > 0:
    ...
    print(f"  Pythia (early training):   {mean_imp:+.1f}% ± {std_imp:.1f}%")
    print(f"  Qwen (fully trained):      -1.0%")

    summary = {
        "step": 5,
        "experiment": "pythia_three_domain",
        "model_id": MODEL_ID,
        "revision": REVISION,
        "domains": DOMAINS,
        "config": {
    ...
    out_path = RESULTS_DIR / "step5_final_summary.json"
    with open(out_path, "w") as f:
...
def main():
    print("=" * 70)
    print("KALAVU: Pythia-410M Three-Domain Specialist Fusion Experiment")
    print("=" * 70)
    print(f"Model:    {MODEL_ID} @ revision={REVISION}")
    print(f"Domains:  {DOMAINS}")
    print(f"Steps:    {MAX_STEPS} per specialist (freeze_layers={FREEZE_LAYERS})")
    print(f"Seeds:    {SEEDS}")
    print(f"Config:   LR={LR}, WD={WEIGHT_DECAY}, BATCH={BATCH_SIZE}×{GRAD_ACCUM}")
    ...
    print("\n[Step 1 complete — commit: git commit -m '[kalavu] step 1: base eval']")

    # Step 2
    step2_train_specialists(tokenizer, all_domain_chunks, device)
    print("\n[Step 2 complete — commit: git commit -m '[kalavu] step 2: specialist training']")

    # Step 3
    step3_results = step3_divergence_check(tokenizer, all_domain_chunks, device)
    print("\n[Step 3 complete — commit: git commit -m '[kalavu] step 3: divergence check']")

    # Step 4
    step4_results = step4_fusion(tokenizer, all_domain_chunks, device, step3_results)
    print("\n[Step 4 complete — commit: git commit -m '[kalavu] step 4: fusion']")

    # Step 5
    step5_final_summary(step1_result, step3_results, step4_results)
    print("\n[Step 5 complete — commit: git commit -m '[kalavu] step 5: final summary']")
