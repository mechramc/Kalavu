...
"""
KALAVAI: Regression Extension P1 — 2-Specialist (Code + Science)
=================================================================
Adds a new (divergence, gain) data point to the divergence-gain regression
by running a 2-specialist cooperative with code and science domains only
(dropping fiction, which is the high-divergence driver at 410M).

Expected outcome:
  - Code div: ~10%,  Science div: ~12%
  - Mean divergence: ~11%  (fills gap between 6.9B=8.73% and 410M-3domain=15.65%)
  - Expected gain: ~4-6% vs best specialist (from regression: 0.82*11 - 2.72 ≈ 2.3%)

Regression value: this is a LOW-DIVERGENCE data point (n=7 total after this run).

Config: identical to kalavai_pythia_experiment.py (same training protocol,
eval protocol, and hyperparameters) except N_EXPERTS=2, DOMAINS=["code","science"].

Usage:
    python experiments/kalavai_regression_p1_2domain.py
"""
...
# ============================================================================
# Config — identical to main 410M experiment except DOMAINS
# ============================================================================
...
# P1: 2 specialists — code + science ONLY (drop fiction to lower mean divergence)
DOMAINS = ["code", "science"]
N_EXPERTS = len(DOMAINS)

RESULTS_DIR = Path("results/regression_extension")
CHECKPOINT_DIR = Path("checkpoints/regression_extension/p1_2domain")
...
# ============================================================================
# Data loading — identical loaders to kalavai_pythia_experiment.py
# ============================================================================
...
# ============================================================================
# N-Expert MoE (generalised, works for any N >= 2)
# ============================================================================

class NExpertMoE(nn.Module):
    """
    Sequence-level MoE over N specialist models.
    Router: linear layer mapping mean-pooled hidden state to N gates.
    Specialists are frozen; only the router is trained.
    Logit-space combination (standard in MoE architectures).
    """
    def __init__(self, specialists: list, hidden_size: int):
        ...
        # Linear router (matches paper's "near-oracle with linear router" result)
    ...
    def forward(self, input_ids, labels=None):
        ...
        # Router: mean of all specialist hidden states
        h_avg = torch.stack(all_h, dim=0).mean(dim=0)   # (B, H)
        gates = torch.softmax(self.router(h_avg), dim=-1)  # (B, N)

        # Logit-space weighted combination
        fused = sum(
...
def eval_all_domains(model, held_out_by_domain: dict, device: str,
    ...
    """Evaluate model on each domain separately, return per-domain losses + equal-weight avg."""
    losses = {}
...
def run_seed(seed: int, all_domain_chunks: dict, device: str) -> dict:
    ...
    # --- Train specialists ---
    specialists = {}
    spec_losses_by_domain = {}  # per-specialist losses on own domain (for divergence)
    ...
    # Divergence = improvement on own domain relative to base
    ...
    print(f"\n[Seed {seed}] Router distribution: {router_dist}")
    ...
    result = {
        ...
        "n_experts": N_EXPERTS,
        "domains": DOMAINS,
        "experiment": "regression_p1_2domain",
        "eval_batch_size": 4,
...
def main():
    print("=" * 70)
    print("KALAVAI Regression Extension P1: 2-Domain (Code + Science)")
    print("=" * 70)
    print(f"Model:    {MODEL_ID} @ {REVISION}")
    print(f"Domains:  {DOMAINS}")
    print(f"Seeds:    {SEEDS}")
    print(f"Goal:     Fill regression gap ~8-13% mean divergence")
    ...
    # Load and split data once (all seeds share same chunks)
    print("\nLoading data...")
    loaders = {
    ...
    for domain, loader_fn in loaders.items():
        ...
        if len(train_c) < 1500:
            print(f"  WARNING: {domain} has <1500 train chunks — increase N_SAMPLES_PER_DOMAIN")
    ...
    for seed in SEEDS:
        ...
        out_path = RESULTS_DIR / f"p1_2domain_seed{seed}.json"
    ...
    summary_path = RESULTS_DIR / "p1_2domain_summary.json"
