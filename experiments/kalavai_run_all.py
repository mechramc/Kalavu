...
"""
KALAVAI: Master Orchestrator — Full Experiment Campaign
======================================================
...
Order:
  ...
  1. Maturity sweep 410M (kalavai_pythia_maturity_sweep.py)
  2. Pythia-1B main result (kalavai_pythia_1b_experiment.py)
  3. Pythia-1B maturity sweep (kalavai_pythia_1b_maturity_sweep.py)
  4. 5-domain specialist scaling (kalavai_pythia_5domain_experiment.py)
  5. Hero figure generation
  6. Results audit

Run with: python kalavai_run_all.py
"""
...
SCRIPTS = [
    ("experiments/kalavai_pythia_maturity_sweep.py",    "maturity sweep 410M"),
    ("experiments/kalavai_pythia_1b_experiment.py",     "Pythia-1B main result"),
    ("experiments/kalavai_pythia_1b_maturity_sweep.py", "Pythia-1B maturity sweep"),
    ("experiments/kalavai_pythia_5domain_experiment.py","5-domain specialist scaling"),
]
...
def generate_hero_figure():
    ...
    try:
        ...
        ax1.axvspan(0, 20, alpha=0.06, color="green", label="KALAVAI target")
        ...
        fig.suptitle("KALAVAI: Cooperative LLM Fusion — Key Results",
...
def run_audit():
    ...
    audit_script = Path("experiments/kalavai_results_audit.py")
    if audit_script.exists():
        run_script("experiments/kalavai_results_audit.py", "Results audit")
    else:
...
def print_final_summary():
    ...
    log("\n" + "="*70)
    log("KALAVAI COMPLETE EXPERIMENT INVENTORY")
    log("="*70)
...
def main():
    log("="*70)
    log("KALAVAI MASTER ORCHESTRATOR — FULL EXPERIMENT CAMPAIGN")
    log("="*70)
    ...
    run_git_commit("[kalavai] monolithic baseline + downstream benchmarks complete")
    run_git_push()
    ...
    for script_name, description in SCRIPTS:
        ...
        if not success:
            ...
            run_git_commit(f"[kalavai] PARTIAL: {description} failed — partial results saved")
            run_git_push()
            sys.exit(1)

        # Commit after each experiment
        run_git_commit(f"[kalavai] {description} complete")
        run_git_push()
    ...
    run_git_commit("[kalavai] all experiments complete — hero figure generated")
    run_git_push()
    ...
    run_audit()
    run_git_commit("[kalavai] results audit complete")
    run_git_push()

    # Print final summary
    print_final_summary()

    log("\n[kalavai] experiment campaign complete — ready for paper writing")
