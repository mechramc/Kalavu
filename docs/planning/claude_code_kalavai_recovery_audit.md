# KALAVAI: Reconstruct Lost Qwen Experiment Scripts + Full Audit

## What Was Lost

Two files were permanently lost when Claude Code wiped the drive:

1. `kalavai_qwen_experiment.py` — the original 2-domain Qwen experiment
2. `kalavai_qwen_experiment_5domain.py` — the 5-domain variant

Both produced verified results that are central to the paper. Check if results JSONs still exist in `kalavai_experiment_results/` or `kalavai_checkpoints*/` first — they may contain enough info to validate the reconstruction.

---

## File 1: `kalavai_qwen_experiment.py`
...
### Checkpoint Saving
Save specialist checkpoints and the router to `kalavai_checkpoints/`. Save results to a JSON file.

---

## File 2: `kalavai_qwen_experiment_5domain.py`
...
domains = {
    ...
    "legal": {
        # Check kalavai_checkpoints_5domain/ for the exact dataset used.
...
If `kalavai_checkpoints_5domain/` has a config or results JSON that records which datasets were used, use those exact datasets. Otherwise use reasonable alternatives and note the substitution.
...
echo "=== Checkpoints ==="
ls -la kalavai_checkpoints/ kalavai_checkpoints_5domain/ 2>/dev/null
...
- [ ] `kalavai_experiment.py` — synthetic proof-of-concept (should be on GitHub from before)
- [ ] `kalavai_held_out_validation.py` — held-out validation (+60.7% result, just recovered)
- [ ] `kalavai_qwen_experiment.py` — 2-domain Qwen (RECONSTRUCT NOW)
- [ ] `kalavai_qwen_experiment_5domain.py` — 5-domain Qwen (RECONSTRUCT NOW)
- [ ] `kalavai_qwen_divergent_domains.py` — code vs fiction, 1000 steps (currently running)
- [ ] Results JSONs for each experiment

### Check the running experiment:
The `kalavai_qwen_divergent_domains.py` background job should still be running or may have finished. Check:
...
============================
Synthetic 25M (held-out):     +60.7% ± 0.7% [3 seeds] — kalavai_held_out_validation.py
Qwen 2-domain (in-dist):     +17.15% — kalavai_qwen_experiment.py
Qwen 5-domain (in-dist):     +38.24% — kalavai_qwen_experiment_5domain.py  
Qwen divergent (held-out):   [PENDING or result] — kalavai_qwen_divergent_domains.py
...
## Do NOT
- Do not modify `kalavai_experiment.py` (original synthetic script) — ground truth
- Do not modify `kalavai_held_out_validation.py` — just recovered, produces +60.7%
- Do not modify `kalavai_qwen_divergent_domains.py` — currently running
